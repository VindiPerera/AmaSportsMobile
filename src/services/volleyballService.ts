...
import { ApiSuccessResponse, VolleyballProfileFormValues, VolleyballProfileResponse } from '../types';

function idOrNull(value: string): number | null {
  return parseIntOrNull(value);
}

function toPayload(values: VolleyballProfileFormValues) {
  return {
...
export const volleyballService = {
  async fetchProfile() {
    const { data } = await apiClient.get<ApiSuccessResponse<VolleyballProfileResponse>>(
      '/player/volleyball-profile'
    );
    return data.data;
  },

  async saveProfile(values: VolleyballProfileFormValues) {
    const { data } = await apiClient.put<ApiSuccessResponse<VolleyballProfileResponse>>(
      '/player/volleyball-profile',
      toPayload(values)
    );
    return data.data;
