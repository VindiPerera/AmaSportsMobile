...
const EMPTY_CAREER_ROW = {
  ...
  runs: '', catches: '', third_place: '', second_place: '', champion: '',
};
...
export default function ElleProfileScreen() {
  ...
  const { control, handleSubmit, reset, setValue, getValues, watch } = useForm<ElleProfileFormValues>({
  ...
  useEffect(() => {
    (async () => {
      try {
        await ensureLoaded();
        const [profile, elleProfile] = await Promise.all([
          playerService.fetchProfile(),
          elleService.fetchProfile(),
        ]);
        ...
        setError('Could not load your Elle profile. Please try again.');
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const onSubmit = async (values: ElleProfileFormValues) => {
    if (!fullName.trim()) {
  ...
  if (isViewing) {
    ...
    const mappedRecent = (formValues.recent_matches || []).map((m) => ({
      ...
      opponent: m.opponent,
      scoreOrStat: m.runs ? `${m.runs} Runs (${m.catches || 0} Catches)` : '--',
  ...
  return (
    <ScreenContainer edges={['bottom']} scroll>
      ...
      <View style={styles.topModeBar}>
        <Text style={styles.topModeTitle}>Editing Elle Profile</Text>
      ...
      <View style={styles.headerRow}>
        ...
        <View style={styles.headerRowItem}>
          <Dropdown label="Sport" value="elle" onChange={() => {}} options={[{ label: 'Elle', value: 'elle' }]} disabled />
        </View>
      </View>
      ...
      <Button label="Save Elle Profile" onPress={handleSubmit(onSubmit)} loading={isSaving} style={styles.submitButton} />
    </ScreenContainer>
  );
}
