...
const EMPTY_RECENT_FIGHT_ROW = {
  fight_date: '', opponent: '', venue: '', weight_class_id: '', win: false, lost: false, place: '',
};

const EMPTY_FORM: BoxingProfileFormValues = {
  born: '', age: '', height: '', weight: '', weight_class_id: '', current_ranking: '',
  college_university: '', teams: [], career_stats: [], recent_fights: [],
};
...
export default function BoxingProfileScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        await ensureLoaded();
        const [profile, boxingProfile] = await Promise.all([
          playerService.fetchProfile(),
          boxingService.fetchProfile(),
        ]);
        ...
        initLogos(
          (boxingProfile as any).team_logos,
          (boxingProfile as any).college_logo_url
        );
        ...
        reset({
          ...
          recent_fights: boxingProfile.recent_fights.map((row) => ({
            ...mapRow(row, ['fight_date', 'opponent', 'venue', 'weight_class_id', 'place']),
            win: Boolean(row.win),
            lost: Boolean(row.lost),
          })) as unknown as BoxingProfileFormValues['recent_fights'],
        });
      } catch {
        setError('Could not load your Boxing profile. Please try again.');
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const onSubmit = async (values: BoxingProfileFormValues) => {
    if (!fullName.trim()) {
    ...
    >
      ...
      <View style={[sportStyles.sectionCard, shadows.sm]}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerRowItem}>
            <Dropdown label="Sport" value="boxing" onChange={() => {}} options={[{ label: 'Boxing', value: 'boxing' }]} disabled />
          </View>
        </View>
      <View style={styles.headerRow}>
        <View style={styles.headerRowItem}>
      ...
      <View style={styles.headerRow}>
        <View style={styles.headerRowItem}>
          <Controller
            ...
            render={({ field: { value, onChange } }) => (
              <Dropdown label="Weight Class" value={value} onChange={onChange} options={weightClassOptions} />
            )}
          />
        </View>
        <View style={styles.headerRowItem}>
