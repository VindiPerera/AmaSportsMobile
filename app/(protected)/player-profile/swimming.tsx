...
import { ApiError, PickedImage, SwimmingProfileFormValues } from '../../../src/types';
...
export default function SwimmingProfileScreen() {
  ...
  } = useSportLogos('swimming');

  const { control, handleSubmit, reset, setValue, getValues, watch } = useForm<SwimmingProfileFormValues>({
  ...
  useEffect(() => {
    (async () => {
      try {
        await ensureLoaded();
        const [profile, swimmingProfile] = await Promise.all([
          playerService.fetchProfile(),
          swimmingService.fetchProfile(),
        ]);
        ...
        initLogos(
          (swimmingProfile as any).team_logos,
          (swimmingProfile as any).college_logo_url
        );
        ...
        reset({
          ...
          career_stats: swimmingProfile.career_stats.map((row) =>
            mapRow(row, Object.keys(EMPTY_CAREER_ROW))
          ) as unknown as SwimmingProfileFormValues['career_stats'],
          recent_events: swimmingProfile.recent_events.map((row) =>
            mapRow(row, Object.keys(EMPTY_RECENT_EVENT_ROW))
          ) as unknown as SwimmingProfileFormValues['recent_events'],
        });
      } catch {
        setError('Could not load your Swimming profile. Please try again.');
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const onSubmit = async (values: SwimmingProfileFormValues) => {
    if (!fullName.trim()) {
    ...
    try {
      ...
      setError(
        err instanceof ApiError
          ? err.message
          : 'Could not save your Swimming profile. Please check your entries and try again.'
      );
    } finally {
      setIsSaving(false);
    }
