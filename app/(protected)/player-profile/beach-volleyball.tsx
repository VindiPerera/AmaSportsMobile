...
import { beachVolleyballService } from '../../../src/services/beachVolleyballService';
...
const EMPTY_FORM: BeachVolleyballProfileFormValues = {
...
export default function BeachVolleyballProfileScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        await ensureLoaded();
        const [profile, beachVolleyballProfile] = await Promise.all([
          playerService.fetchProfile(),
          beachVolleyballService.fetchProfile(),
        ]);
        ...
        reset({
          ...
          recent_matches: beachVolleyballProfile.recent_matches.map((row) => ({
            ...mapRow(row, ['match_date', 'opponent', 'venue', 'set_1', 'set_2', 'set_3']),
            win: Boolean(row.win),
            lost: Boolean(row.lost),
          })) as unknown as BeachVolleyballProfileFormValues['recent_matches'],
        });
      } catch {
        setError('Could not load your Beach Volleyball profile. Please try again.');
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const onSubmit = async (values: BeachVolleyballProfileFormValues) => {
    if (!fullName.trim()) {
  ...
  return (
    <ScreenContainer edges={['bottom']} scroll>
      ...
      <View style={styles.topModeBar}>
        <Text style={styles.topModeTitle}>Editing Beach Volleyball Profile</Text>
      ...
      <View style={styles.headerRow}>
        ...
        <View style={styles.headerRowItem}>
          <Dropdown label="Sport" value="beach-volleyball" onChange={() => {}} options={[{ label: 'Beach Volleyball', value: 'beach-volleyball' }]} disabled />
        </View>
      </View>
      ...
      <Button label="Save Beach Volleyball Profile" onPress={handleSubmit(onSubmit)} loading={isSaving} style={styles.submitButton} />
    </ScreenContainer>
  );
}
