...
export default function MatchDetailScreen() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToLiveScore(matchIdNum, (updatedScore) => {
      setLiveData(updatedScore);
    });
  ...
  // Handle manual score update for simulation
  ...
  return (
    <ScreenContainer edges={['bottom']} scroll>
      ...
      {/* 3b. Player Score Card — striker/non-striker from the admin's
          ball-by-ball cricket scorer (Admin\LiveScoreController's cricket
          partial). Only present once an admin has actually started scoring
          with the full two-innings scorer (older-shape matches won't have
          `team_a`/`team_b`). */}
      ...
      {/* 3c. Batting / Bowling stats — full scorecard per innings, mirroring
          the admin scorer's tables. team_a/team_b are fixed slots ("bats in
          innings 1"/"innings 2"), so both get their own block once that
          innings has actually started; bowling figures for an innings come
          from the OPPOSING side's bowlers. */}
      ...
      {/* 4. Roster — the admin panel's Player Registration step (ID Number
          / Name / Photo per side). Only present on the detail response. */}
